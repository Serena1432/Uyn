			...
			message.channel.send(embed).then(msg => {
				var interval = setInterval(function() {
					...
					if (playerUtb == playerTeam.members.length && opponentUtb == opponentTeam.members.length) {
						...
						embed.setFooter((message.mentions.users.size ? message.mentions.users.first().username : "The opponent") + " is the winner!\nYou lost " + parseInt(15 *  (2 + maxLevel * 0.35) *  opponentTeam.members.length * (1 + client.economyManager[message.author.id].streaks * 0.1)) + " " + client.config.currency + (message.mentions.users.size ? (" and " + message.mentions.users.first().username + " got " + parseInt(15 *  (2 + maxLevel * 0.35) *  opponentTeam.members.length * (1 + client.economyManager[message.author.id].streaks * 0.1)) + " " + client.config.currency + " and " + parseInt(5 * (2 + maxLevel * 0.15) *  opponentTeam.members.length * (1 + client.economyManager[message.author.id].streaks * 0.1)) + " EXP") : "") + "...\nYou lost your streak of " + (client.economyManager[message.author.id].streaks) + " wins...");
					...
					if (!end && opponentUtb != opponentTeam.members.length || res == "draw") msg.edit(embed);
						...
						if (res == "win") coins += parseInt(15 *  (2 + maxLevel * 0.35) *  opponentTeam.members.length * (1 + client.economyManager[message.author.id].streaks * 0.1));
						else coins -= parseInt(15 *  (2 + maxLevel * 0.35) *  opponentTeam.members.length * (1 + client.economyManager[message.author.id].streaks * 0.1));
						client.economyManager[message.author.id].coins = encrypt(coins.toString());
						if (res == "win") {
							var rExp = parseInt(5 * (2 + maxLevel * 0.15) *  opponentTeam.members.length * (1 + client.economyManager[message.author.id].streaks * 0.1));
							for (var i = 0; i < client.economyManager[message.author.id].team.members.length; i++) {
								var waifu;
								for (var j = 0; j < client.economyManager[message.author.id].waifus.length; j++) {
									if (client.economyManager[message.author.id].waifus[j].id == client.economyManager[message.author.id].team.members[i]) {
						...
						request.post({url: process.env.php_server_url + "/EconomyManager.php", formData: {
							...
							if (!error && response.statusCode == 200 && body.includes("Success")) {
								if (message.mentions.users.size) {
									...
									if (res == "win") {
										if (parseInt(15 *  (2 + maxLevel * 0.35) *  opponentTeam.members.length * (1 + client.economyManager[message.author.id].streaks * 0.1)) < coins) coins -= parseInt(15 *  (2 + maxLevel * 0.35) *  opponentTeam.members.length * (1 + client.economyManager[message.author.id].streaks * 0.1));
										else coins = 0;
									}
									else coins += parseInt(15 *  (2 + maxLevel * 0.35) *  opponentTeam.members.length * (1 + client.economyManager[message.author.id].streaks * 0.1));
									...
									request.post({url: process.env.php_server_url + "/EconomyManager.php", formData: {
										...
										if (!error && response.statusCode == 200 && body.includes("Success")) {
											...
											if (client.channels.cache.get(client.config.logChannel)) client.channels.cache.get(client.config.logChannel).send("**ID:** " + result, new Discord.MessageEmbed()
												...
												.setAuthor(message.mentions.users.first().username + " has just " + (res == "lose" ? "won" : "lost") + " " + parseInt(15 *  (2 + maxLevel * 0.35) *  opponentTeam.members.length * (1 + client.economyManager[message.author.id].streaks * 0.1)) + " because of " + (res == "lose" ? "winning" : "losing") + " a battle.", message.mentions.users.first().avatarURL({size: 128}))
												.setTimestamp()
											);
									...
									if (client.channels.cache.get(client.config.logChannel)) client.channels.cache.get(client.config.logChannel).send("**ID:** " + result, new Discord.MessageEmbed()
										.setColor(Math.floor(Math.random() * 16777215))
										.setAuthor(message.author.username + " has just " + (res == "win" ? "won" : "lost") + " " + parseInt(15 *  (2 + maxLevel * 0.35) *  opponentTeam.members.length * (1 + client.economyManager[message.author.id].streaks * 0.1)) + " because of winning a battle.", message.author.avatarURL({size: 128}))
										.setTimestamp()
									);
									else console.log("Cannot get the log channel.");
									msg.edit(embed);
